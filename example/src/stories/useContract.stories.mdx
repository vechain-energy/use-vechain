...
If `transaction` is given as option, only `comment` is valid as another option. The other options are for a transction and can not be defined for each single clause.  
